...
namespace app\module\codegen\service;
...
",
    ...
    public function saveGenConfig(string $tableName, array $formData): bool
    {
        ...
        Db::transaction(function () use (
            ...
            if ($tableId <= 0) {
                throw new BusinessException('生成配置保存失败');
            }
    ...
    public function downloadZip(array $tableNames, string $pageType = 'classic', string $type = 'ts'): array
    {
        ...
        if (!class_exists('ZipArchive')) {
            throw new BusinessException('ZipArchive 扩展未启用');
        }
        ...
        if ($opened !== true) {
            throw new BusinessException('创建 zip 失败');
        }
    ...
    private function renderFromTemplate(string $relativePath, array $vars): string
    {
        ...
        if (!is_file($file)) {
            throw new BusinessException('模板不存在 ' . $relativePath);
        }
