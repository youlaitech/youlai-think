...
enum ResultCode: string implements IResultCode
{
    ...
    public function getMsg(): string
    {
        return match ($this) {
            ...
            self::USER_ERROR => '用户端错�?,

            self::USER_REGISTRATION_ERROR => '用户注册错误',
            self::USER_NOT_AGREE_PRIVACY_AGREEMENT => '用户未同意隐私协�?,

            self::VERIFICATION_CODE_INPUT_ERROR => '校验码输入错�?,

            self::USER_LOGIN_EXCEPTION => '用户登录异常',
            self::ACCOUNT_NOT_FOUND => '用户账户不存�?,
            self::ACCOUNT_FROZEN => '用户账户被冻�?,
            self::USER_PASSWORD_ERROR => '用户名或密码错误',

            self::ACCESS_TOKEN_INVALID => '访问令牌无效或已过期',
            self::REFRESH_TOKEN_INVALID => '刷新令牌无效或已过期',

            self::USER_VERIFICATION_CODE_ERROR => '验证码错�?,
            self::USER_VERIFICATION_CODE_ATTEMPT_LIMIT_EXCEEDED => '用户验证码尝试次数超�?,
            self::USER_VERIFICATION_CODE_EXPIRED => '用户验证码过�?,

            self::ACCESS_PERMISSION_EXCEPTION => '访问权限异常',
            self::ACCESS_UNAUTHORIZED => '访问未授�?,

            self::USER_REQUEST_PARAMETER_ERROR => '用户请求参数错误',
            self::INVALID_USER_INPUT => '无效的用户输�?,
            self::REQUEST_REQUIRED_PARAMETER_IS_EMPTY => '请求必填参数为空',
            self::PARAMETER_FORMAT_MISMATCH => '参数格式不匹�?,

            self::USER_REQUEST_SERVICE_EXCEPTION => '用户请求服务异常',
            self::REQUEST_CONCURRENCY_LIMIT_EXCEEDED => '请求并发数超出限�?,
            ...
            self::THIRD_PARTY_SERVICE_ERROR => '调用第三方服务出�?,
            self::INTERFACE_NOT_EXIST => '接口不存�?,
            self::DATABASE_SERVICE_ERROR => '数据库服务出�?,
            self::DATABASE_EXECUTION_SYNTAX_ERROR => '数据库执行语法错�?,
            self::INTEGRITY_CONSTRAINT_VIOLATION => '违反了完整性约�?,
